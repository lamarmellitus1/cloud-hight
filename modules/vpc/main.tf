# create vpc
resource "aws_vpc" "cloudhightVPC" {
...
resource "aws_internet_gateway" "cloudhight-igw" {
  vpc_id    = aws_vpc.cloudhightVPC.id
  tags      = {
...
resource "aws_route_table" "cloudhight-public_route_table" {
  vpc_id       = aws_vpc.cloudhightVPC.id

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.cloudhight-igw.id
  }

  tags       = {
    Name     = "cloudhight-Public-rt"
  }
}
#==========================================================================
#===========================================================================
...
resource "aws_route_table_association" "pub-sub-1a_route_table_association" {
  ...
  route_table_id      = aws_route_table.cloudhight-public_route_table.id
}
...
resource "aws_subnet" "cloudhight-private_sub_3a" {
  vpc_id                   = aws_vpc.cloudhightVPC.id
  cidr_block               = var.cloudhight-private_sub_3a_cidr
  availability_zone        = data.aws_availability_zones.available_zones.names[0]
...
resource "aws_subnet" "cloudhight-private_sub_4b" {
  vpc_id                   = aws_vpc.cloudhightVPC.id
...
# create private data subnet pri-sub-5a(database subnets)
resource "aws_subnet" "cloudhight-private_sub_5a" {
  vpc_id                   = aws_vpc.cloudhightVPC.id
...
resource "aws_subnet" "cloudhight-private_sub_6b" {
  vpc_id                   = aws_vpc.cloudhightVPC.id
