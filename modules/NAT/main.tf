...
resource "aws_eip" "cloudhight-eip-nat-a" {
  domain = "vpc"
  tags = {
    Name = "cloudhight-eip-nat-a"
  }
}

# allocate elastic ip. this eip will be used for the nat-gateway in the public subnet pub-sub-2-b
resource "aws_eip" "cloudhight-eip-nat-b" {
domain = "vpc"
  tags = {
    Name = "cloudhight-eip-nat-b"
  }
}

# create nat gateway in public subnet pub-sub-1a
resource "aws_nat_gateway" "cloudhight-pub_nat-a" {
  allocation_id = aws_eip.cloudhight-eip-nat-a.id
  subnet_id     = var.pub_sub_1a_id

  tags = {
    Name = "cloudhight-nat-a"
  }
...
resource "aws_nat_gateway" "cloudhight-nat-b" {
  allocation_id = aws_eip.cloudhight-eip-nat-b.id
  subnet_id     = var.pub_sub_2b_id

  tags = {
    Name = "cloudhight-nat-b"
  }
...
resource "aws_route_table" "pri-rt-a" {
  ...
  route {
    ...
    nat_gateway_id = aws_nat_gateway.cloudhight-pub_nat-a.id
  }

  tags = {
    Name = "cloudhight-Primary-rt-a"
  }
}
...
resource "aws_route_table" "pri-rt-b" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.cloudhight-nat-b.id
  }

  tags = {
    Name = "cloudhight-pri-rt-b"
  }
}
